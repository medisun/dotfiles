...
if [ -z "$pid" ]; then
   ...
   # using wmctrl get the first visible windows using $pid. 
   # Redirect stderr to /dev/null and only select
   ...
   if [ -z "$wid" ]; then
     ...
     echo "Found vissible window for process $1 with PID: $pid and WID: $wid"
     wmctrl -i -a $wid
   fi
fi
